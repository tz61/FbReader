...
module axi4_fbreader_to_hdmi_v1_0_M00_AXI #(
    ...
    input logic[2:0] out_volume_shift,
  ...
  always_comb begin
    if (drawX[9:1] <= 9'h13F) begin
      ...
      if (doutb_fb1[0] == 1) begin  // draw 2D over 3D canvas
        if (drawX[0] == 0) begin
          R = doutb_fb1[31:24];
          G = doutb_fb1[23:16];
          B = doutb_fb1[15:8];
  ...
  pwm_gen pwm_left (
      ...
      .rst(~M_AXI_ARESETN),
      .din((left_mixer[15:4]>>out_volume_shift)),  // not a better way
      .dout(left_out)
  );
  pwm_gen pwm_right (
      .clk_200mhz(clk_200mhz),
      .rst(~M_AXI_ARESETN),
      .din((right_mixer[15:4]>>out_volume_shift)),
      .dout(right_out)
  );
